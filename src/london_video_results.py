...
def get_london_regressors():
	filenames = ["exp_outputs/london/test_video_algorithm_{}_json.json".format(x) for x in range(1,6)]
	regressors = {}
	patch_counts = {}
	total_counts = {}
	...
	with open("exp_outputs/london/regressors.json", 'wb') as ff:
		json.dump(regressors , ff)
	with open("exp_outputs/london/patch_counts.json", 'wb') as ff:
		json.dump(patch_counts , ff)
	with open("exp_outputs/london/total_counts.json", 'wb') as ff:
		json.dump(total_counts , ff)


def examine_regressors():
	with open("exp_outputs/london/regressors.json", 'r') as f:
		results = json.load(f)
...
def examine_patch_counts():
	with open("exp_outputs/london/patch_counts.json", 'r') as f:
		results = json.load(f)	
	...
	for i in range(9):
		...
		alg_2 = A[1,:,i]
		alg_3 = A[2,:,i]
		a1 = int(round(alg_1.sum()/alg_1.size, 1))
		a2 = int(round(alg_2.sum()/alg_2.size, 1))
		a3 = int(round(alg_3.sum()/alg_3.size, 1))

		mi1 = int(round(alg_1.min(), 1))
		mi2 = int(round(alg_2.min(), 1))
		mi3 = int(round(alg_3.min(), 1))
		ma1 = int(round(alg_1.max(), 1))
		ma2 = int(round(alg_2.max(), 1))
		ma3 = int(round(alg_3.max(), 1))

		print "{} &{} & {} & {}& {} & {}& {} & {}& {} & {} \\\\ \hline".format(i+1,a1, a2, a3, mi1, mi2, mi3, ma1, ma2, ma3)



def examine_total_counts():
	with open("exp_outputs/london/total_counts.json", 'r') as f:
		results = json.load(f)	
	t = []
	for i in range(1,6):
		t.append(results[str(i)])
